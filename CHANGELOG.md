...
* Although it caused no apparent issue, the pieces that were dupplicated from an event of type "repeat" had its parents `hasClones` property set to true (since it was dupplicated and unchanged after that). To make more sense in the pieces, the dupplicates now have the `nowClones` property set to false upon cloning.
